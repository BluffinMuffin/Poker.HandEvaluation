...
{
    public class QualityEvaluatorThreeOfAKind
    {
        ...
        public QualityEvaluatorThreeOfAKind(ICardGroupQualityHelper cardGroupQualityHelper, IClassifiedCardsBuilder classifiedCardsBuilder)
        {
        ...
        public ClassifiedCards Evaluate(IEnumerable<ICard> cards, IEvaluationOptions options)
        {
            ...
            return _classifiedCardsBuilder.Build(three, CardGroupQualityEnum.ThreeOfAKind, options, allCards.Except(three).OrderByDescending(x => x.Value).Take(2));
        }
    }
}